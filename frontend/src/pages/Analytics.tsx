...
export default function Analytics() {
  ...
  const safeLabels = analyticsLabels || {};

  // Mock analytics data with safety guards
  const analyticsData = {
    overview: {
      totalAlerts: 1247,
      alertsChange: 12.5,
  ...
  const getScenarioLabel = (scenario: string) => {
    if (!safeLabels) return scenario;
    switch (scenario) {
      case 'exam_pressure': return safeLabels.examPressure || '考试压力';
      case 'isolation_bullying': return safeLabels.isolationBullying || '孤立霸凌';
      case 'self_harm': return safeLabels.selfHarm || '自伤行为';
      case 'teacher_verbal_abuse': return safeLabels.teacherVerbalAbuse || '教师言语暴力';
      case 'cyber_tracking': return safeLabels.cyberTracking || '网络跟踪';
  ...
  const getChangeIcon = (change: number) => {
    try {
      ...
      console.warn('Error rendering change icon:', error);
      return <div className="w-4 h-4" />;
    }
  };
    ...
    return (
      <div className="mx-auto max-w-7xl p-6">
        <div className="flex items-center justify-center h-64">
          <div className="animate-pulse text-slate-400">加载中 / Loading...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:gap-8">
      ...
      {/* Alert Trends Chart - Full Width Section */}
      <AlertTrends useMockData={true} />

      {/* Alert Types Breakdown and Classroom Performance - Two Column Layout */}
